...
export const WinkUpdates: React.FC<WinkUpdatesProps> = ({ sentWinks, onAddUpdate }) => {
    ...
    const sortedWinks = [...sentWinks].sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime());
                            ...
                            <div key={wink.id} className="bg-brand-surface p-4 sm:p-5 rounded-2xl shadow-lg border border-brand-secondary-200/30">
                                <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-start">
                                    <div className="flex-grow min-w-0">
                                        <p className="text-sm text-brand-text-secondary">{wink.timestamp.toLocaleDateString()}</p>
                                        ...
                                        {wink.updates.map((update, index) => (
                                            <div key={index} className="flex items-start gap-3 text-sm">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-xs text-brand-text-secondary/70 mt-1">{update.timestamp.toLocaleDateString()}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
